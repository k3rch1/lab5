...
template<class T>
void list_sequence<T>::remove(size_t index) {
    items.extract(index);
}
...
template<class T>
template<class U>
list_sequence<std::pair<T, U>> list_sequence<T>::zip(const list_sequence<U>& other) const {
    if (size() != other.size())
        throw std::invalid_argument("size mismatch");
    list_sequence<std::pair<T, U>> result;
...
std::pair<list_sequence<T>, list_sequence<U>> unzip(const list_sequence<std::pair<T, U>>& seq) {
    list_sequence<T> first;
    list_sequence<U> second;
